...
Phase       : Day 2/3 - Testbench Vector Update (16-byte array, 4-bit addr)

Pin mapping (per updated RTL, 16 x 8-bit array to fit TT utilization):
    ui_in[7:0]   = write data payload
    uio_in[3:0]  = address (4-bit, 0-15)
    uio_in[4]    = unused / reserved (tie low)
...
NOTE: The array was shrunk from 32x8 to 16x8 to meet Tiny Tapeout
utilization limits. Address width dropped from 5 bits to 4 bits
(uio_in[3:0]); WE/CE/PERSIST retain their original bit positions
(uio_in[5]/[6]/[7]). All test vectors below are constrained to the
valid 0-15 address range -- addresses like 31 no longer exist and
will alias/truncate against a 4-bit bus, which was the root cause
of the prior sequential-readback failures.
"""
...
def _pack_uio(addr=0, we=0, ce=0, persist=0):
    ...
    Address is now 4 bits wide (0-15); bit 4 is unused/reserved."""
    return ((persist & 0x1) << 7) | ((ce & 0x1) << 6) | ((we & 0x1) << 5) | (addr & 0xF)
...
async def test_single_and_sequential_read_write(dut):
    ...
    dut._log.info("Sequential R/W OK across addresses 0, 7, 15")

    # --- Extra coverage: a pseudo-random sweep across all 16 addresses ---
    random.seed(42)
    sweep = {addr: random.randint(0, 255) for addr in range(16)}
    ...
    dut._log.info("Full 16-address sweep OK")
